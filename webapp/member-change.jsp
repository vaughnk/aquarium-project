...
<body>
	<script type="text/javascript">
		function memeberDel() {
			...
			if (ans == true) {
				...
				document.memberChange.action = "memberDelete.do";
				console.log(document.memberChange.action);
			} else {
			...
			<div class="container-fluid pt-4 px-4">
				<div
					...
					<div class="bg-light rounded h-100 p-4">
						...
						<form action="memberUpdate.do" name="memberChange" id="memberChange" method="post">
							<div class="row mb-3">
								<label for="inputEmail3" class="col-sm-2 col-form-label">아이디</label>
								<div class="col-sm-10">
									<input type="text" class="form-control" id="userId"
										name="userId" placeholder="아이디" value="${member.memberId}"
										readonly>
								</div>
							</div>
							<div class="row mb-3">
								...
								<div class="col-sm-10">
									<input type="password" class="form-control" id="password"
										...
										onkeyup="validationPw()" placeholder="비밀번호">
								</div>
							...
							<div class="field birth">
								<div>
									...
									<input type="text" placeholder="출생년" id="userYear"
										name="userYear" value="${member.birthyy}"
										style="padding: 0.375rem 0.75rem; border: 1px solid #ced4da; border-radius: 5px;">
									<input type="text" placeholder="출생월" id="userMonth"
										name="userMonth" value="${member.birthmm}"
										style="padding: 0.375rem 0.75rem; border: 1px solid #ced4da; border-radius: 5px;">
									<input type="text" placeholder="출생일" id="userDay"
										name="userDay" value="${member.birthdd}"
										style="padding: 0.375rem 0.75rem; border: 1px solid #ced4da; border-radius: 5px;">
								</div>
							</div>
							...
							<div class="row mb-3">
								<label for="inputEmail3" class="col-sm-2 col-form-label"
									style="margin-top: 20px;">이메일</label>
								<div class="col-sm-10">
							...
							<div class="row mb-3">
								...
								<div class="col-sm-6 ">
									<button type="submit" class="btn btn-primary"
										onClick="validationCheck()" style="float: right;">회원정보수정</button>
								</div>
								<div class="col-sm-6">
									<button type="submit" class="btn btn-primary"
										onClick="memeberDel()">회원정보삭제 </button>
								</div>
							</div>
						</form>
					</div>
	...
	<%--
		...
		const userName = document.getElementById("userName").value;
			...
			const userBirth = document.getElementById("userYear").value;
			if (userBirth == '') {
				alert("생년을 입력해주세요.");
				return;
			}
